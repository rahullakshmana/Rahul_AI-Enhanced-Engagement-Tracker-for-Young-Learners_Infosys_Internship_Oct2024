...
if not os.path.exists("screenshots_rahul(2)"):
    os.makedirs("screenshots_rahul(2)")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"screenshots_rahul(2)/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpeg"
                if attentive == 'Yes':
        ...
        if (datetime.now() - last_save_time).total_seconds() >= 30:
            df.to_excel('rahul_attendance_with_attention_score(1).xlsx', index=False)
            print("DataFrame saved to 'rahul_attendance_with_attention_score(1).xlsx'")
    ...
    if not df.empty:
        df.to_excel('rahul_attendance_with_attention_score(1).xlsx', index=False)
        print("Final attendance with attentiveness saved to 'rahul_attendance_with_attention_score(1).xlsx'.")
