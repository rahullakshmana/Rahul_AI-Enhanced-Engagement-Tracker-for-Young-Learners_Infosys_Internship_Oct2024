...
if not os.path.exists("screenshots_rahul(0)"):
    os.makedirs("screenshots_rahul(0)")
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    # Save screenshot
                    screenshot_filename = f"screenshots_rahul(0)/Rahul_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
        ...
        if not recognized:
            ...
            cv.putText(frame, 'Not Rahul', (30, 55), cv.FONT_HERSHEY_SIMPLEX, 1,
                       (255, 0, 0), 2, cv.LINE_AA)
    ...
    if not df.empty:
        df.to_excel('rahul_attendance_with_screenshots(0).xlsx', index=False)
        print("Attendance and screenshots saved to 'rahul_attendance_with_screenshots(0).xlsx'.")
