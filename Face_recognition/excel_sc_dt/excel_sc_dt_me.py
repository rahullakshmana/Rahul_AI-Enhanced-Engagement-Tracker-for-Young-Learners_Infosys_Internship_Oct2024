...
if not os.path.exists("screenshots_rahul(1)"):
    os.makedirs("screenshots_rahul(1)")
...
try:
    while True:
        ...
        for face_encoding in face_encodings:
            ...
            if distance < confidence_threshold:
                ...
                if last_recognition_time is None or (now - last_recognition_time) >= entry_duration:
                    ...
                    screenshot_filename = f"screenshots_rahul(1)/Rahul_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                ...
                for face_location in face_locations:
                    ...
                    cv.putText(frame, 'Rahul', (left, top - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5,
        ...
        if (datetime.now() - last_saved_time).total_seconds() >= 30:
            df.to_excel('rahul_attendance_with_screenshots_dt(1).xlsx', index=False)
            print("Data saved to 'rahul_attendance_with_screenshots_dt(1).xlsx'.")
    ...
    if not df.empty:
        df.to_excel('rahul_attendance_with_screenshots_dt(1).xlsx', index=False)
        print("Attendance and screenshots saved to 'rahul_attendance_with_screenshots_dt(1).xlsx'.")
