...
if not os.path.exists("screenshots_rahul(4)"):
    os.makedirs("screenshots_rahul(4)")
...
try:
    while True:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            if distance < 0.6:
                ...
                screenshot_filename = f"screenshots_rahul(4)/{name}_{now.strftime('%Y-%m-%d_%H-%M-%S')}.jpg"
                if attentive:
        ...
        if (datetime.now() - last_save_time) > timedelta(seconds=30):
            df.to_excel('rahul_attendance_with_attention(3).xlsx', index=False)
            print("Data saved to 'rahul_attendance_with_attention(3).xlsx'")
    ...
    if not df.empty:
        df.to_excel('rahul_attendance_with_attention(3).xlsx', index=False)
        print("Final data saved to 'rahul_attendance_with_attention(3).xlsx'")
